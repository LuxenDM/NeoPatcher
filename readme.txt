...
How does NeoPatcher create its patches? Its actually really simple. It takes the original main.lua file and renames it to "core_patched.lua", then creates new prerun.lua and main.lua files based on a template. It then creates a Neoloader declaration INI based on the provided information, and then its done! 

The end result is a plugin that registers itself to Neoloader. When the Neoloader init process occurs, prerun.lua immediately freezes the plugin (this will prevent dependent plugins from ever running, but nothing should be dependent on a patched plugin anyways!), and then patched_core.lua will run during the default plugin loader like normal as long as the plugin is set to load. As a bonus, if Neoloader is not enabled, the plugin will skip all the neoloader stuff and just launch like normal anyways. 
